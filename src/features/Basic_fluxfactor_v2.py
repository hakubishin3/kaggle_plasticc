...
def get_basic_ts(ts_data):
    ...
    agg_results.drop([
        ...
        "flux_ratio_sq_sum_detected1", "flux_ratio_sq_skew_detected1"], axis=1, inplace=True)

    return agg_results


def add_fluxfactor(ts_data, meta_data):
    ts_data["flux_negflg"] = 1
    ts_data.loc[ts_data.flux < 0, "flux_negflg"] = -1
    ts_data["flux"] = abs(ts_data["flux"])

    cosmo = FlatLambdaCDM(H0=70, Om0=0.3, Tcmb0=2.725)
    dist = meta_data.set_index("object_id")["predict_specz"].map(cosmo.distmod).apply(lambda x: x.value)
    dist.name = "dist"
    ts_data = ts_data.set_index("object_id").join(dist, how="inner").reset_index()

    ts_data["flux"] = -2.5 * np.log10(ts_data["flux"]) - ts_data["dist"]
    ts_data["flux"] = 10**(ts_data["flux"] / -2.5)
    ts_data["flux"] = ts_data["flux"] * ts_data["flux_negflg"]
    ts_data["flux_err"] = -2.5 * np.log10(ts_data["flux_err"]) - ts_data["dist"]
    ts_data["flux_err"] = 10**(ts_data["flux_err"] / -2.5)

    ts_data.drop("dist", axis=1, inplace=True)
    ts_data.drop("flux_negflg", axis=1, inplace=True)

    return ts_data


class Basic_fluxfactor_v2(Feature):
    def create_features(self, train_meta, test_meta):
        """
        train: meta-data
        test: meta-data
        """
        self.suffix = "fluxfactor_v2"
        logger = get_module_logger(__name__)

        # make train features
        start = time.time()
        train_ts = pd.read_csv('../../data/input/training_set.csv')
        fe = pd.read_feather("../../data/feature/Predictor_specz_train.ftr")
        train_meta = pd.concat([train_meta, fe], axis=1)
...
class Basic_fluxfactor_passband_v2(Feature):
    def create_features(self, train_meta, test_meta):
        """
        train: meta-data
        test: meta-data
        ...
        total_steps = int(np.ceil(test_row_num / chunks))
        extra_obj_list = test_meta.query("hostgal_photoz != 0")["object_id"].tolist()
        fe = pd.read_feather("../../data/feature/Predictor_specz_test.ftr")
        test_meta = pd.concat([test_meta, fe], axis=1)
...
if __name__ == '__main__':
    ...
    #f = Basic_fluxfactor_v2('../../data/feature/')
    #f.run(train_meta, test_meta).save()

    # make Basic_fluxfactor_passband features
    f = Basic_fluxfactor_passband_v2('../../data/feature/')
    f.run(train_meta, test_meta).save()
