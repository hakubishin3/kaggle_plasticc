...
def get_aggregations():
    agg_dict = {
        'fluxdiffdiff': ['min', 'max', 'mean', 'median', 'std', 'skew']
    }
    return agg_dict
...
def add_features_to_agg(df):
    df['fluxdiffdiff_diff'] = df['fluxdiffdiff_max'] - df['fluxdiffdiff_min']
    df['fluxdiffdiff_dif2'] = (df['fluxdiffdiff_max'] - df['fluxdiffdiff_min']) / df['fluxdiffdiff_mean']

    # add detected == 1 feature
    df['fluxdiffdiff_diff_detected1'] = df['fluxdiffdiff_max_detected1'] - df['fluxdiffdiff_min_detected1']
    df['fluxdiffdiff_dif2_detected1'] = (df['fluxdiffdiff_max_detected1'] - df['fluxdiffdiff_min_detected1']) / df['fluxdiffdiff_mean_detected1']
    return df
...
def add_fluxfactor(ts_data, meta_data):
    ...
    diff_result = ts_data.groupby(["object_id", "passband"])[["fluxdiff", "mjddiff"]].diff()
    diff_result.columns = ["fluxdiffdiff", "mjddiffdiff"]
    ts_data = pd.concat([ts_data, diff_result], axis=1).dropna()
    ts_data.drop(["flux", "mjd", "fluxdiff", "mjddiff"], axis=1, inplace=True)

    return ts_data


class Basic_fluxdiffdiff(Feature):
...
def get_basic_ts_passband(ts_data):
    ...
    for passband in passband_list:
        agg_results[f'fluxdiffdiff_diff_{passband}'] = agg_results[f'fluxdiffdiff_max_{passband}'] - agg_results[f'fluxdiffdiff_min_{passband}']
        agg_results[f'fluxdiffdiff_dif2_{passband}'] = (agg_results[f'fluxdiffdiff_diff_{passband}']) / agg_results[f'fluxdiffdiff_mean_{passband}']

    agg_results.reset_index(drop=False, inplace=True)
    return agg_results


class Basic_fluxdiffdiff_passband(Feature):
    def create_features(self, train_meta, test_meta):
        """
        train: meta-data
        test: meta-data
...
if __name__ == '__main__':
    ...
    # make Basic_fluxdiffdiff features
    f = Basic_fluxdiffdiff('../../data/feature/')
    f.run(train_meta, test_meta).save()

    # make Basic_fluxdiffdiff_passband features
    f = Basic_fluxdiffdiff_passband('../../data/feature/')
    f.run(train_meta, test_meta).save()
